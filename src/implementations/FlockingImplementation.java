...
public class FlockingImplementation implements AnimationControls
{
    ...
    public void checkLeaderCollision(Character leader1)
    {
        for (Character leader2 : leaderList)
        {
            if (leader1 != leader2 && PVector.sub(leader1.position, leader2.position).mag() < radiusOfSeparation) {
                leader1.velocity.x *= -2;
                leader1.velocity.y *= -2;
                leader1.orientation += Math.PI;
                leader1.move();
            }
        }
